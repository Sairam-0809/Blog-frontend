...
function Food() {
  const [foodLatestData, setFoodLatestData] = useState([]);
  const [foodTopData, setFoodTopData] = useState([]);
  useEffect(() => {
    const fetchData = async () => {
      try {
        const response = await axios.get('https://blogbackend-muak.onrender.com/blogrouter/getblogdata/foodlatest');
        setFoodLatestData(response.data);
      } catch (error) {
  ...
  useEffect(() => {
    const fetchData1 = async () => {
      try {
        const response = await axios.get('https://blogbackend-muak.onrender.com/blogrouter/getblogdata/foodtop');
        setFoodTopData(response.data);
      } catch (error) {
        console.error('Error fetching data:', error);
      }
    };
